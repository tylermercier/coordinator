module Coordinator
  class Base
    ...
    def add_task(skill, task)
      @queues.find_by_skill(skill).add_task(task)
    end

    def add_priority_task(skill, task)
      @queues.find_by_skill(skill).add_priority_task(task)
    end

    def remove_task(skill, task)
      @queues.find_by_skill(skill).remove_task(task)
    end
    ...
    def set_capacity(skill, capacity)
      @queues.find_by_skill(skill).set_capacity(capacity)
    end

    def info(skill)
      queue = @queues.find_by_skill(skill)
      {
        "name" => queue.name,
